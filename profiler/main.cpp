...
int main(int argc, char *argv[]) {
    ...
    //    std::cout << "file[0]: " << file[0].c_str() << std::endl;
    ...
    // line count must come first so that it doesn't instrument 
    // the expr_stmt added by mainReport() and funcCount()
    code.lineCount(profileName[0]);           //Count line invocations 
    code.mainReport(profileName);             //Add in the report
    code.funcCount(profileName[0]);           //Count function invocations
    code.condCount(profileName[0]);           //Count function invocations
    ...
    for (unsigned i = 1; i < file.size(); ++i) {  //Read rest of the files.
        ...
        code.funcCount(profileName[i]);        //Count function invocations
        code.condCount(profileName[i]);        //Count function invocations
