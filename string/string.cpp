...
//Your resetCapacity is wrong given how you are using it in operator+.  And there is a bug in operator+ given how you implement resetCapacity.

 // I suggest using String(int, const char[]) to define your result.   Make it length() + rhs.length().   Also do not modify result.stringSize in + as this is done in the ctor.   This is currently a bug in your operator+.   You set stringSize in + but that is not what you allocate in resetCapacity.
